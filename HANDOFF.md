...
- **THE DEPARTMENT SYSTEM** (`departments.ts`, pure + 6 tests): colors in
  ...
  Geometry in `departmentBorders()` (departments.ts, pure + 6 tests in
  departmentBorders.test.ts): paint room polygons into a ~720-wide grid,
  box-dilate to close wall-thickness gaps (joinPx = min(planW,planH)*0.004
  — neighbours merge, corridors don't), 4-connected components, edge-walker
  contour trace per component, RDP simplify, back to plan px. MapCanvas
  renders them via the `outlines` prop (`.deptborder` paths, non-scaling
  5px stroke, pointer-events none) after the plan image, under labels;
  rooms keep stroke=fill. The same borders show in Max Space dept mode.
  3D view outlines: not yet (follow-up).

429 tests green. E2E-verified: zoom survives taps, bar live-updates
(including the remove case), save round-trip, 2-room assign with stored
color, cluster borders on both maps (demo naturally produced the split
case: one department → two contours, the neighbour → one).
