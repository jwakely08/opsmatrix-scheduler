...
- **THE DEPARTMENT SYSTEM** (`departments.ts`, pure + 6 tests): colors in
  ...
  MapCanvas's new `strokeFor` prop draws each room's OUTLINE in its
  department color while the fill stays the schedule's; legend appends
  outline swatches. 3D view outlines: not yet (follow-up).

423 tests green. E2E-verified: zoom survives taps, bar live-updates
(including the remove case), save round-trip, 2-room assign with stored
color, outline strokes on the map.
