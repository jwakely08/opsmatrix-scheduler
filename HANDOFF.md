...
- **Ground truth fixtures**: `test-fixtures/Test_project_-_1st_Floor.dxf` + `Test_project_Statistics.csv` are **Josh's REAL magicplan exports (READ-ONLY — never regenerate/substitute)**. History note: earlier fixtures were synthetic stand-ins I generated (fully disclosed + audited on 2026-08-05); Josh then provided the real files and everything was rebuilt/validated against them. **170 vitest tests green (`npm test`)**: parsers, geometry (incl. damaged-scan robustness), compute engine of the old React app, schedule-doc generation (§10), the AI plan reader (§11), and the CAD room-list importer + workload engine (§12a).
...
- **Daily Schedules** (`src/pro/FloorCareApp.tsx`, engine in `src/pro/floorcare.ts`, store `opsmatrix_fusion_floorcare`): the five floor-care tasks (`floorCare: true` in rules — Machine Scrubbing/auto-scrub, Dust Mopping, Burnishing, + NEW Machine Sweeping & Machine Carpet Cleaning) are scheduled HERE. Builder: equipment icon tiles → manufacturer → model dropdowns from `src/pro/equipment.ts` (~120 machines transcribed from Josh's four-brand OEM reference sheets — Tennant/Advance/TASKI scrubbers, four-brand burnishers, four-brand wet-carpet; **no sweeper sheet provided yet → custom-entry only**; dust mops use ISSA-style width rates 18–72", editable; unrated catalog models take a manual rate; every category accepts custom machines). The selected machine's OEM rate prices ITS task on THAT schedule (`stopMinutes`: equipment rate wins, Scope rate is the fallback). 1–4 technicians with per-tech stop assignment (`FcStop.techKey`), per-tech + total `HoursBar`s (yellow <7.5h, green 7.5–8.1, red >8.1 — the bar also renders on every Max Schedules card). Eligible rooms only (fcEligible: rooms whose tasks include floor-care work + carpeted rooms); click order = running order. **Confirm ships into Max Schedules** (`shipToSchedules`): a real schedule with `floorCareId` (edit redirects to Floor Care — map legend editing blocked) and `floorCareMinutes` (equipment-priced total; `scheduleMinutes` honors it); roomTasks carry the floor-care tasks only, never the base clean, so coverage/Unassigned-Tasks reporting stays truthful. Deleting a floor-care schedule unships its counterpart.
...
```
npm test                                      # 170 tests must stay green
