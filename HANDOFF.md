...
## 17. OPSMATRIX HOTELS — A SEPARATE REPO, ON PURPOSE (2026-09-15)

OpsMatrix Hotels is Josh's hotel operations concept, branded OpsMatrix but a **separate product in its own repository: `jwakely08/opsmatrix-hotels`** (live at `https://jwakely08.github.io/opsmatrix-hotels/?demo=1`). Josh's decision after the engine audit (`docs/ENGINE_AUDIT.md`, §8): hotels and healthcare differ too much to share an app, and a hotel product must never be wired into healthcare data — so **no shared code, data, login or deploy**, not even a shared page on this site. The hotel repo carries its own HANDOFF, tests and Pages workflow.

For THIS repo that means: never add hotel code, hotel links or hotel data here; never import from the hotel repo; the audit doc stays because it classifies this engine. A hotel build briefly lived on the `claude/opsmatrix-hotels-engine-dbadsj` branch (commits 9fa9c17…9bb4bc9) before it moved out; the branch now carries only the audit and these notes. Test count here is back to 302.
