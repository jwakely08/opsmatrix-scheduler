...
- **Ground truth fixtures**: `test-fixtures/Test_project_-_1st_Floor.dxf` + `Test_project_Statistics.csv` are **Josh's REAL magicplan exports (READ-ONLY — never regenerate/substitute)**. History note: earlier fixtures were synthetic stand-ins I generated (fully disclosed + audited on 2026-08-05); Josh then provided the real files and everything was rebuilt/validated against them. **147 vitest tests green (`npm test`)**: parsers, geometry (incl. damaged-scan robustness), compute engine of the old React app, schedule-doc generation (§10), the AI plan reader (§11), and the CAD room-list importer + workload engine (§12a).
...
- **Department identity ≠ name** (critical): identity from dept code (or name); a numeric code is never displayed as a name; blank-named-but-coded departments stay SEPARATE with stable display labels "Blank Department N" (`blankDeptLabels`) that are never saved as names; Cost Center is preserved for analytics but NEVER promoted to a department NAME. When a file has NO department columns at all (Josh's E-building export), a populated Cost Center becomes the department IDENTITY (`departmentKey: "cc:<code>"`) so rooms billed together group together as separate Blank Departments — a junk cost center like "-" is no evidence, so those rooms stay unassigned. Renaming a Blank Department (Space Validation → search its cost center → select all → type the name) keeps the same grouping.
...
```
npm test                                      # 147 tests must stay green
