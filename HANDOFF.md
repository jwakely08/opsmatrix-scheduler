...
## 17. OPSMATRIX HOTELS — THE SEPARATE CONCEPT BUILD (2026-09-14)

**A second product on the same repo, deliberately separate from everything above.** Josh's decision after the engine audit (`docs/ENGINE_AUDIT.md` §8): hotels and hospitals are two products with the OpsMatrix name, separate logins later, no shared switch, no hookups until the concept earns it. The hospital app is untouched — the only shared-file change is the `hotels` entry in `vite.config.ts`.

- **Page**: `hotels.html` → `src/hotels/main.tsx`. Deploys with the normal `npm run build` → GitHub Pages at `…/hotels.html` (`?demo=1` reseeds the demo hotel). Own store `opsmatrix_hotels_v1` + `opsmatrix_hotels_api_key`; never reads `opsmatrix_v7` or any fusion key.
- **Theme**: ivory/sand/gold tokens in `src/hotels/theme.ts` + `theme.css`; Cormorant Garamond + Manrope vendored in `public/fonts/` (hard rule 7 — no CDN). Serif room numbers, verdict as a small lamp, hatched Unknown. `#design` renders every component.
- **Model** (`store.ts`): spaces with polygons on a plan, suites as **clusters** (one guest-facing verdict = worst member), stacked status layers per room (occupancy, condition, engineering, guest flags, inspection, ready path, GM override), work orders, assignments, reservations, ledger, forecast, staff. Signals carry `at` (last confirmed → freshness) and `since` (value changed).
- **Ready Confidence** (`verdict.ts`, 16 tests): stale/missing required signal → Unknown, never green; feed disconnected → all Unknown; OOO / do-not-walk / blocking WO / dirty → red; clean-not-inspected / non-blocking WO / inspection older than the condition change → hold; GM override lifts a hold only, with a reason, logged. Freshness per signal in Settings (default 15 min).
- **Mutations** (`actions.ts`): the ONE path — every change lands in the status layers, the queue and the report ledger. Screens and capture both call it.
- **Ambient capture** (`capture/`): `parse.ts` local grammar (26 utterance tests — room numbers incl. spoken words, conditions, faults by category, hand-offs, holds, inspection calls, notes, handover lines; the room carries across clauses); `ai.ts` Claude parser (raw fetch, user key on device, `output_config.format` JSON schema, same headers as the hospital reader — request shape tested), falls back to local on any failure; `apply.ts` intents → actions (end-to-end test on the 412 example). Walk mode: hold-to-talk (Web Speech), typed fallback, readback receipt cards, tap to leave one out, Apply, two-tap inspect + photo, reassign. Settings → auto-apply above a confidence.
- **Reports** (`reports.ts`): shift handover (windowed, grouped by room, searchable), daily rooms, scar map (work orders + failed inspections, 30/90 days), weekly labor vs standard (`Handover.tsx`); PDF = print CSS, share = Web Share API / clipboard.
- **Labor** (`labor.ts`): daily demand = departures × departure min + stayovers × stayover min + turndown (if on) + public-area passes + travel; 14-day plan + FTE; `todayWork` from the status layers; contiguous boards (floor + number order, filled to the productive limit) vs scattered (call-down order) with the difference in minutes and dollars; variance. Forecast pasted as `date, arrivals, departures, stayovers`.
- **Manual adapter** (`roomsImport.ts`): CSV/paste → spaces, suites, status signals; PMS shorthand (VD/VC/VI/OD) understood; upsert by room number.
- **Demo** (`demo.ts`): "The Meridian", 38 guest rooms on floors 3–4 (Suite 320/420 = parlor + bedroom clusters), lobby level public areas, 13 arrivals with promised times, every verdict level on screen incl. exactly one honestly-stale room (311), OOO 407, failed inspection 315, work-order history, ledger, forecast. Demo data older than 2 h (or a stamp bump) reseeds on load; real data (no `demoStamp`) is never touched.
- **Verified**: 366 vitest green (64 hotel tests); Playwright screenshots at 393px and 1380px of every view; scripted e2e of the 412 sentence through Walk mode → map drawer → front desk → ledger, zero page errors.
- **Open**: hook a real plan through the existing floor-plan reader (the pieces are shared); roster import; logins/RLS; Mews adapter; write-back. `docs/PILOT_DAY_ONE.md` is the customer-facing day-one script.
