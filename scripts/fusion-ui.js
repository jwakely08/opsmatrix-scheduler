...
(function () {
  ...
  // "Map Scheduler" entry in the classic sidebar → the Pro page
  function ensureNavLink() {
    if (document.getElementById("fusion-pro-nav")) return;
    var navBtns = document.querySelectorAll("button");
    var anchor = null;
    for (var i = 0; i < navBtns.length; i++) {
      if ((navBtns[i].textContent || "").trim() === "Max Schedules") { anchor = navBtns[i]; break; }
    }
    if (!anchor || !anchor.parentNode) return;
    var link = document.createElement("button");
    link.id = "fusion-pro-nav";
    link.type = "button";
    ...
    label.textContent = "Map Scheduler";
    link.appendChild(icon);
    link.appendChild(label);
    link.addEventListener("click", function () { window.location.href = "./maps.html"; });
