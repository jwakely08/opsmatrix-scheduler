...
(function () {
  ...
  function ensureNavLink() {
    ...
    for (var i = 0; i < navBtns.length; i++) {
      ...
      if (t === "Max Space" && !b.getAttribute("data-fusion-space-wired")) {
        ...
        b.addEventListener("click", function (e) {
          ...
          if (sessionStorage.getItem("fusion-goto-plans") === "1") return; // calibration flow
          if (sessionStorage.getItem("fusion-goto-addplan") === "1") return;
          e.preventDefault();
          e.stopPropagation();
    ...
    if (anchor) {
      ...
      openAddPlanIfRequested();
    }
  }

  /**
   * The calibration flow (measure a room you know + border detection) is the
   * archive's own Add Floor Plan path. Two flags drive the hop:
   *   fusion-goto-plans   → click Floor Plans once it exists
   *   fusion-goto-addplan → then click Add Floor Plan / Upload First Plan
   */
  var addPlanTries = 0;
  function openAddPlanIfRequested() {
    if (sessionStorage.getItem("fusion-goto-addplan") !== "1") return;
    if (addPlanTries++ > 40) { sessionStorage.removeItem("fusion-goto-addplan"); return; }
    var btns = document.querySelectorAll("button");
    for (var i = 0; i < btns.length; i++) {
      var t = (btns[i].textContent || "").trim();
      if (t === "Add Floor Plan" || t === "Upload First Plan") {
        sessionStorage.removeItem("fusion-goto-addplan");
        btns[i].click();
        return;
      }
    }
  ...
  function openFloorPlansIfRequested(floorPlansBtn) {
    if (sessionStorage.getItem("fusion-goto-plans") === "1") {
      sessionStorage.removeItem("fusion-goto-plans");
      floorPlansBtn.click();
      return;
    }
  ...
  function ensureSpaceScreen() {
    // roads into the ARCHIVE's Max Space (Floor Plans / calibration only):
    // ?fp=1 deep link, or the calibration flags. Room lists live in the hub.
    var wantSpace = /[?&]fp=1/.test(window.location.search) ||
      sessionStorage.getItem("fusion-goto-plans") === "1";
  ...
  /** hop into the archive's own Add Floor Plan flow (trace + calibrate) */
  function goToCalibrate() {
    sessionStorage.setItem("fusion-goto-plans", "1");
    sessionStorage.setItem("fusion-goto-addplan", "1");
    // if we're already on the archive's Max Space, the observer takes it from
    // here; otherwise click the nav (the rewire lets it through on the flags)
    if (!clickNav("Max Space")) {
      window.location.href = "./classic.html?calibrate=1";
    }
  }
  ...
  function boot() {
    // ?calibrate=1 (from the hub's plan-upload dialog): straight into the
    // archive's Add Floor Plan flow — trace a known room, calibrate, detect
    ...
    if (!sessionStorage.getItem("fusion-goto-page") &&
        !/[?&](fp|calibrate)=1/.test(window.location.search) &&
        sessionStorage.getItem("fusion-goto-plans") !== "1") {
