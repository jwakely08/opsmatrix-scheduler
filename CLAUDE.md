...
## OpsMatrix Hotels lives in its OWN repo

`jwakely08/opsmatrix-hotels` is a separate product (Josh's rule: no shared code, data, login or deploy between hotel and healthcare). Never add hotel code, links or data here; never import from there. Background: HANDOFF §17, `docs/ENGINE_AUDIT.md` §8.

## Commands

```
npm test               # vitest — 302 tests, must stay green
